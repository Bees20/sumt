...
/opt/mongodb_exporter/config:
  file.append:
    - text: |
        OPTIONS=mongodb://'pmm:{{ pillar['password'] }}'@localhost:27017

copy__files_mongo:
  file.managed:
    - name: /opt/mongodb_exporter/mongodb_exporter
    - source: http://{{ reposerver }}/prometheus/mongodb_exporter
    - user: node_exporter
    - group: node_exporter
    - skip_verify: True
    - mode: 0755
...
/etc/systemd/system/mongodb_exporter.service:
  file.append:
    - text: |
        ...
        ExecStart=/opt/mongodb_exporter/mongodb_exporter -mongodb.uri $OPTIONS

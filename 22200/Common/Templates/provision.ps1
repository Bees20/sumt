  ...
  function GetLargestVersion
{
    ...
    createTempFile($FileName);				
		...
		Copy-Item -Confirm:$false -Destination "{{PACKAGE_ROOT}}\{{BUILD}}\{{ROLE}}.zip" -Force -Path "$Folder";
    ...
    Write-Host "CLEAN UP  PACKAGE" -foregroundcolor "black" -backgroundcolor "yellow";
		...
		if(ValidateUnzip -zippath "$Folder" -folderpath "{{PACKAGE_ROOT}}\{{BUILD}}\{{ROLE}}")
		{
  ...
  else
  {
    ...
    #Step 1: Provision all the necessary roles on this server
    Import-Module "{{PACKAGE_ROOT}}\{{BUILD}}\{{ROLE}}\Installer\{{ROLE}}_Workflows.psm1" -ArgumentList "{{TEMP}}", "AddCluster{{WORKFLOW}}_{{ROLE}}_{{SERVER}}_{{CLUSTER}}";
		
			
				Write-Host "RUNNING {{WORKFLOW}}  for role {{ROLE}}" -foregroundcolor "black" -backgroundcolor "yellow";
			$query = "EXEC AddorUpdateServerVersion_v17_1 '{{SERVER}}' , '{{ROLE}}', '{{BUILD}}', '' , '{{WORKFLOW}}', 'STARTED'";
		try{
	...
	$query = "SELECT dbo.GET_PS_INSTALL_PARAMETERS( '{{SERVER}}', '{{CLUSTER}}', '{{ASSOCIATED_CLUSTER}}' , '{{WORKFLOW}}', '{{ROLE}}', NULL )";
		try{
		$InstallParameters =
		SumTInvoke-SqlReader -Query $query -ServerInstance "{{UDAC_SERVER}}" -Database "{{UDAC_DB}}" -Username "{{UDAC_USER}}" -Password "{{UDAC_PASS}}" -QueryTimeout 0
    ...
    if(![string]::IsNullOrWhiteSpace($TARGET_PATCH_VERSION))
    {
		  ...
		  $patchVersionCheck = DoesPatchVersionExists -Package '{{BUILD}}' -ParameterXmlString $config -Role '{{ROLE}}' -Version $TARGET_PATCH_VERSION -SiteKey null    
    ...
    $query = "EXEC AddorUpdateServerVersion_v17_1 '{{SERVER}}' , '{{ROLE}}', '{{BUILD}}', '' , '{{WORKFLOW}}', 'COMPLETED'";
		try{
		SumTInvoke-SqlReader -Query $query -ServerInstance "{{UDAC_SERVER}}" -Database "{{UDAC_DB}}" -Username "{{UDAC_USER}}" -Password "{{UDAC_PASS}}" -QueryTimeout 0
		...
		break;
				 ...
				 Write-Host "COMPLETED {{WORKFLOW}} for role {{ROLE}}" -foregroundcolor "black" -backgroundcolor "green";
			
    }
    else
    {
    $query = "EXEC AddorUpdateServerVersion_v17_1 '{{SERVER}}' , '{{ROLE}}', '{{BUILD}}', '' , '{{WORKFLOW}}', '$obj_status : $obj_message : $obj_errordetail'";
		try{
		SumTInvoke-SqlReader -Query $query -ServerInstance "{{UDAC_SERVER}}" -Database "{{UDAC_DB}}" -Username "{{UDAC_USER}}" -Password "{{UDAC_PASS}}" -QueryTimeout 0
		...
		break;
				...
				Write-Host "ERROR occured while running {{WORKFLOW}}  for role : {{ROLE}} : $obj_message : $obj_errordetail" -foregroundcolor "black" -backgroundcolor "red";

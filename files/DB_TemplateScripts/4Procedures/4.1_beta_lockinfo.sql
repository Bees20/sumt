...
/****** Object:  StoredProcedure [dbo].[beta_lockinfo]    Script Date: 7/26/2021 6:15:45 AM ******/
SET ANSI_NULLS ON
GO
...
----------------------------------------------------------------------------
-- Archive mode. We save data to dba..lockinfo which we create if
-- needed.
----------------------------------------------------------------------------
do_archive:
IF @archivemode <= 0 AND object_id('dba..lockinfo') IS NOT NULL
   DROP TABLE dba..lockinfo
IF object_id('dba..lockinfo') IS NULL
BEGIN
   IF @debug = 1
   BEGIN
      SELECT @ms = datediff(ms, @now, sysdatetime())

...
/****** Object:  StoredProcedure [dbo].[DatabaseBackup]    Script Date: 7/26/2021 6:16:47 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
  ...
  SET @StartMessage = @StartMessage + ', @Encrypt = ' + ISNULL(CAST(@Encrypt AS nvarchar),'NULL')
  SET @StartMessage = @StartMessage + ', @EncryptionType = ' + ISNULL(CAST(@EncryptionType AS nvarchar),'NULL')
  SET @StartMessage = @StartMessage + ', @EncryptionKey = ' + ISNULL(CAST(@EncryptionKey AS nvarchar),'NULL')
  ...
  BEGIN
    ...
    BEGIN
      ...
      SET @CurrentDirectory = @Directory + CASE WHEN RIGHT(@Directory,1) = '\' THEN '' ELSE '\' END + REPLACE(CAST(SERVERPROPERTY('servername') AS nvarchar),'\','$') + '\' + UPPER(@CurrentBackupType) + '\' + @CurrentDatabaseNameFS + CASE WHEN @CopyOnly = 'Y' THEN '_COPY_ONLY' ELSE '' END
      ...
      BEGIN
        ...
        SET @CurrentFilePath = @CurrentDirectory + '\' + @CurrentDatabaseNameFS + CASE WHEN @BackupSoftware IS NOT NULL THEN '_' + UPPER(@BackupSoftware) ELSE '' END + CASE WHEN @CopyOnly = 'Y' THEN '_COPY_ONLY' ELSE '' END + '_' + REPLACE(REPLACE(REPLACE((CONVERT(nvarchar,@CurrentDate,120)),'-',''),' ','_'),':','') + CASE WHEN @NumberOfFiles > 1 AND @NumberOfFiles <= 9 THEN '_' + CAST(@CurrentFileNumber AS nvarchar) WHEN @NumberOfFiles >= 10 THEN '_' + RIGHT('0' + CAST(@CurrentFileNumber AS nvarchar),2) ELSE '' END + '.' + @CurrentFileExtension

        IF LEN(@CurrentFilePath) > 259
        BEGIN
          SET @CurrentFilePath = @CurrentDirectory + '\' + LEFT(@CurrentDatabaseNameFS,CASE WHEN (LEN(@CurrentDatabaseNameFS) + 259 - LEN(@CurrentFilePath) - 3) < 20 THEN 20 ELSE (LEN(@CurrentDatabaseNameFS) + 259 - LEN(@CurrentFilePath) - 3) END) + '...' + CASE WHEN @BackupSoftware IS NOT NULL THEN '_' + UPPER(@BackupSoftware) ELSE '' END + CASE WHEN @CopyOnly = 'Y' THEN '_COPY_ONLY' ELSE '' END + '_' + REPLACE(REPLACE(REPLACE((CONVERT(nvarchar,@CurrentDate,120)),'-',''),' ','_'),':','') + CASE WHEN @NumberOfFiles > 1 AND @NumberOfFiles <= 9 THEN '_' + CAST(@CurrentFileNumber AS nvarchar) WHEN @NumberOfFiles >= 10 THEN '_' + RIGHT('0' + CAST(@CurrentFileNumber AS nvarchar),2) ELSE '' END + '.' + @CurrentFileExtension
        END
